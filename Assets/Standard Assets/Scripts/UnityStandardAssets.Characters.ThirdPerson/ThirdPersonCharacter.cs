...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
		...
		private void ScaleCapsuleForCrouching(bool crouch)
		{
			...
			{
				Ray arg_BD_0 = new Ray(this.m_Rigidbody.position + Vector3.up * this.m_Capsule.radius * 0.5f, Vector3.up);
				float maxDistance = this.m_CapsuleHeight - this.m_Capsule.radius * 0.5f;
				if (Physics.SphereCast(arg_BD_0, this.m_Capsule.radius * 0.5f, maxDistance, -1, QueryTriggerInteraction.Ignore))
				{
					this.m_Crouching = true;
					return;
				}
		...
		private void PreventStandingInLowHeadroom()
		{
			if (!this.m_Crouching)
			{
				Ray arg_6E_0 = new Ray(this.m_Rigidbody.position + Vector3.up * this.m_Capsule.radius * 0.5f, Vector3.up);
				float maxDistance = this.m_CapsuleHeight - this.m_Capsule.radius * 0.5f;
				if (Physics.SphereCast(arg_6E_0, this.m_Capsule.radius * 0.5f, maxDistance, -1, QueryTriggerInteraction.Ignore))
				{
					this.m_Crouching = true;
				}
			}
